...
export default function Resize() {
  ...
  const [mode, setMode] = usePersistedState<Mode>('resize.mode', 'pixel')
  ...
  const start = () => {
    ...
    if (mode === 'pixel' && w <= 0 && h <= 0) {
      setError('请至少填写宽度或高度其中一项')
      return
    }
    if (mode === 'percent' && (p <= 0 || p > 1000)) {
      setError('缩放比例需在 1 - 1000 之间')
      return
    }
